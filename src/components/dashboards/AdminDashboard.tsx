...
import { UserPlus, CreditCard, CalendarPlus, Settings, BarChart3, Upload, Star, Users, AlertTriangle, Coins, Loader2, Edit3, Trash2 } from "lucide-react";
...
import { db, serverTimestamp } from '@/lib/firebase/firebase'; // Import db and serverTimestamp
...
interface UserData {
    ...
    status: 'Ativo' | 'Pendente' | 'Inativo'; // Example statuses
    coins?: number;
}
...
interface EventData {
    ...
    status?: 'Ativo' | 'Agendado' | 'Concluído'; // Calculated or set
    linkedCards?: string[];
}
...
const cardSchema = z.object({
    ...
    copiesAvailable: z.coerce.number().optional().nullable().transform(val => val === undefined || val === null || isNaN(val) ? null : Number(val)),
    eventId: z.string().optional().nullable(),
    price: z.coerce.number().min(0, "Preço deve ser positivo.").default(0),
});
...
export function AdminDashboard() {
        ...
        const unsubscribe = onSnapshot(collection(db, "events"), (snapshot) => {
            const eventsData = snapshot.docs.map(doc => {
                ...
                return {
                    id: doc.id,
                    ...data,
                    startDate: data.startDate?.toDate ? data.startDate.toDate() : new Date(data.startDate), // Convert Timestamp to Date
                    endDate: data.endDate?.toDate ? data.endDate.toDate() : new Date(data.endDate) // Convert Timestamp to Date
                } as EventData;
            });
    ...
    // Reset forms when editing state changes
    useEffect(() => {
        if (editingCard) cardForm.reset(editingCard); else cardForm.reset({ name: "", rarity: "Comum", imageUrl: "https://placehold.co/200x280.png", available: true, copiesAvailable: null, eventId: "", price: 0 });
    }, [editingCard, cardForm]);

    useEffect(() => {
        if (editingEvent) eventForm.reset(editingEvent); else eventForm.reset({ name: "", startDate: undefined, endDate: undefined, bonusMultiplier: 1, linkedCards: []});
    ...
    async function onCardSubmit(values: z.infer<typeof cardSchema>) {
        ...
        try {
            if (editingCard) {
                await updateDoc(doc(db, "cards", editingCard.id), values);
                toast({ title: "Carta Atualizada!", description: `A carta "${values.name}" foi atualizada.` });
            } else {
                await addDoc(collection(db, "cards"), values);
    ...
    async function deleteCard(cardId: string) {
     ...
     async function onEventSubmit(values: z.infer<typeof eventSchema>) {
        ...
        try {
            const eventData = {
                ...values,
                // Firestore Timestamps are handled automatically if values.startDate/endDate are JS Date objects
            };
            if (editingEvent) {
    ...
    async function onUserSubmit(values: z.infer<typeof studentSchema>) {
        ...
        try {
            if (editingUser) {
                // Note: Updating email/RA via admin might require re-auth or backend functions for security.
                // For now, we update Firestore. Auth email changes are more complex.
                await updateDoc(doc(db, "users", editingUser.id), {
                    name: values.name,
                    ra: values.ra,
                    email: values.email, // Be cautious with direct email updates here.
                    role: values.role,
                });
                toast({ title: "Usuário Atualizado!", description: `Os dados de "${values.name}" foram atualizados.` });
            } else {
                // User creation is typically handled via Auth (RegisterForm). This form could be for editing or pre-populating.
                // For simplicity, this admin form won't create new Auth users, only Firestore profiles.
                // This would ideally be linked to an Auth user creation process or a more robust admin SDK usage.
                // await addDoc(collection(db, "users"), {...values, createdAt: serverTimestamp() });
                // toast({ title: "Usuário Adicionado!", description: `O usuário "${values.name}" foi adicionado.` });
     ...
     async function handleStudentRegistration(data: any) {
        // This function would handle CSV upload and processing.
     ...
     const getEventStatus = (event: EventData): 'Ativo' | 'Agendado' | 'Concluído' => {
        const now = new Date();
        if (event.endDate < now) return 'Concluído';
        if (event.startDate > now) return 'Agendado';
        return 'Ativo';
    };
                                 ...
                                 <Form {...userForm}>
                                    <form onSubmit={userForm.handleSubmit(onUserSubmit)} className="space-y-4 p-4 border rounded-lg shadow-sm mb-6 bg-secondary/30">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Button variant="outline" onClick={() => setEditingUser(null)} disabled={isLoading}>Cancelar</Button>
                                        </div>
                                    </form>
                                </Form>
                            )}
                                        ...
                                        {users.map((user) => (
                                            <TableRow key={user.id}>
                                                ...
                                                <TableCell className="text-right">
                                                    ...
                                                    {/* Add delete user functionality with caution */}
                                                </TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                        ...
                        <CardContent className="space-y-6">
                             <Form {...cardForm}>
                                <form onSubmit={cardForm.handleSubmit(onCardSubmit)} className="space-y-4 p-4 border rounded-lg shadow-sm bg-secondary/30">
                                    ...
                                    <FormField control={cardForm.control} name="copiesAvailable" render={({ field }) => (<FormItem><FormLabel>Cópias Disponíveis (Opcional)</FormLabel><FormControl><Input type="number" placeholder="Deixe em branco para ilimitado" {...field} value={field.value ?? ''} onChange={e => field.onChange(e.target.value === '' ? null : Number(e.target.value))} /></FormControl><FormDescription className="text-xs">Para cartas com estoque limitado na loja.</FormDescription><FormMessage /></FormItem>)} />
                                     {/* TODO: Add Event ID selector if cards are event-specific */}
                                    <FormField control={cardForm.control} name="available" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm bg-background"><div className="space-y-0.5"><FormLabel>Disponível na Loja?</FormLabel><FormDescription className="text-xs">Se esta carta pode ser comprada na loja.</FormDescription></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
                                    <div className="flex gap-2">
                                        ...
                                        {editingCard && <Button variant="outline" onClick={() => { setEditingCard(null); cardForm.reset(); }} disabled={isLoading}>Cancelar Edição</Button>}
                                    </div>
                                </form>
                            </Form>
                             <Separator />
                            <h3 className="text-lg font-semibold">Cartas Registradas ({cards.length})</h3>
                             <ScrollArea className="h-[400px]">
                                <Table>
                                     <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>Raridade</TableHead><TableHead>Preço</TableHead><TableHead>Disponível</TableHead><TableHead>Qtde.</TableHead>{/* <TableHead>Evento Vinculado</TableHead> */}<TableHead className="text-right">Ações</TableHead></TableRow></TableHeader>
                                    <TableBody>
                                        {cards.map((card) => (
                                            <TableRow key={card.id}>
                                                <TableCell className="font-medium flex items-center gap-2">
                                                    <img src={card.imageUrl} alt={card.name} data-ai-hint="card game" className="h-10 w-auto rounded-sm border" />
                                                    {card.name}
                                                </TableCell>
                                                ...
                                                {/* <TableCell>{card.eventId ?? '-'}</TableCell> */}
                                                <TableCell className="text-right">
                 ...
                 return (
                    <Card>
                        ...
                        <CardContent className="space-y-6">
                             <Form {...eventForm}>
                                <form onSubmit={eventForm.handleSubmit(onEventSubmit)} className="space-y-4 p-4 border rounded-lg shadow-sm bg-secondary/30">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {editingEvent && <Button variant="outline" onClick={() => { setEditingEvent(null); eventForm.reset(); }} disabled={isLoading}>Cancelar Edição</Button>}
                                    </div>
                                </form>
                             </Form>
                             <Separator />
                            ...
                            <ScrollArea className="h-[400px]">
                                <Table>
                                    <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>Período</TableHead><TableHead className="text-center">Multiplicador</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader>
                                     <TableBody>
                                        {events.map((event) => {
            ...
            <div className="border-b">
                <nav className="-mb-px flex space-x-1 md:space-x-4 overflow-x-auto" aria-label="Tabs">
                    ...
                    <Button variant={activeTab === 'settings' ? 'default' : 'ghost'} onClick={() => setActiveTab('settings')} className="whitespace-nowrap py-3 px-2 md:px-1 border-b-2 font-medium text-sm data-[state=active]:border-primary data-[state=active]:text-primary">
                     ...
                     <Button variant={'ghost'} onClick={() => alert("Página de relatórios em desenvolvimento!")} className="whitespace-nowrap py-3 px-2 md:px-1 border-b-2 font-medium text-sm" disabled>
